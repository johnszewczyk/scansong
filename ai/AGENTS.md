...
- `MediaScannerKit` is the single scanner implementation shared by every host.
- Keep the library independent of AppKit, SwiftUI, Electron, playlists, and host-owned databases.
- The `media-scan` executable is a versioned JSONL process boundary and must keep stdout machine-readable.
